...
public class SnakeMovement1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (nextObj != null && !isPaused) // && !levelInfo.GetComponent<LevelInfo>().isPaused
        {
            ...
            nextObj.GetComponent<TailPartsMovementMult1>().trajectory.Enqueue(nextPosForTailPart);
        }
    ...
    public void AddNewTailPart()
    {
        ...
        levelInfo.GetComponent<LevelInfoMult>().score1++;
        ...
        tailParts[tailParts.Count - 1].GetComponent<TailPartsMovementMult1>().firstPos = firstPosForTailPart;
    }
